	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if (r1.DestructiveAssessment == nil) != (r2.DestructiveAssessment == nil) {
				t.Fatalf("destructive assessment nil mismatch: %#v vs %#v", r1.DestructiveAssessment, r2.DestructiveAssessment)
			}
			if r1.DestructiveAssessment != nil {
				if *r1.DestructiveAssessment != *r2.DestructiveAssessment {
					t.Fatalf("destructive assessment mismatch: %#v vs %#v", *r1.DestructiveAssessment, *r2.DestructiveAssessment)
				}
			}
			if (r1.PrivacyAssessment == nil) != (r2.PrivacyAssessment == nil) {
				t.Fatalf("privacy assessment nil mismatch: %#v vs %#v", r1.PrivacyAssessment, r2.PrivacyAssessment)
			}
			if r1.PrivacyAssessment != nil {
				if *r1.PrivacyAssessment != *r2.PrivacyAssessment {
					t.Fatalf("privacy assessment mismatch: %#v vs %#v", *r1.PrivacyAssessment, *r2.PrivacyAssessment)
				}
			}
		})
	}
...
func TestPropertyAssessmentMatchConsistency(t *testing.T) {
	...
	for _, cmd := range commands {
		res := guard.Evaluate(cmd, guard.WithDestructivePolicy(guard.InteractivePolicy()))
		if len(res.Matches) > 0 && !hasAnyAssessment(res) {
			t.Fatalf("matches present but assessments nil for %q", cmd)
		}
		if !hasAnyAssessment(res) {
			if res.Decision != guard.Allow {
				t.Fatalf("nil assessments must imply allow for %q; got %s", cmd, res.Decision)
			}
			if len(res.Matches) != 0 {
				t.Fatalf("nil assessments must imply no matches for %q; got %d", cmd, len(res.Matches))
			}
		}
	}
}
...
func TestDeterministicPacksMetadataCopyIsolation(t *testing.T) {
	...
	if !slices.EqualFunc(p1, p2, func(a, b guard.PackInfo) bool {
		return a.ID == b.ID && a.Name == b.Name && a.Destructive.Count == b.Destructive.Count && a.Privacy.Count == b.Privacy.Count && a.Both.Count == b.Both.Count
	}) {
