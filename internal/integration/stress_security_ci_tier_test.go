	...
	for w := 0; w < workers; w++ {
		...
		go func() {
			...
			for i := w; i < len(entries); i += workers {
				...
				if !hasAnyAssessment(r) && r.Decision != guard.Allow {
					errCh <- fmt.Errorf("entry[%d] nil assessments with non-allow decision=%s", i, r.Decision)
					return
				}
			}
		}()
...
func TestStressHighVolumeFuzzSeedRunner(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		...
		if !hasAnyAssessment(r) && r.Decision != guard.Allow {
			t.Fatalf("seed[%d] nil assessments with non-allow decision=%s", i, r.Decision)
		}
	}
}
	...
	for i := 0; i < goroutines; i++ {
		...
		go func() {
			...
			for j := 0; j < perGoroutine; j++ {
				...
				if !hasAnyAssessment(r) && r.Decision != guard.Allow {
					errCh <- fmt.Errorf("goroutine=%d iter=%d nil assessments with %s", i, j, r.Decision)
					return
				}
			}
		}()
