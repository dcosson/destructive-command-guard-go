...
func rsyncPack() packs.Pack {
	return packs.Pack{
		...
		Destructive: []packs.Rule{
			{ID: "rsync-delete", Severity: sevHigh, Confidence: confHigh, Reason: "rsync --delete removes destination files that are not present in source", Remediation: "Run with --dry-run first and verify source/destination paths", Match: packs.And(packs.Name("rsync"), packs.Or(packs.RawTextContains("--delete"), packs.Flags("--remove-source-files")))},
		},
	}
}
