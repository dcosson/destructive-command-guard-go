...
func communicationPack() packs.Pack {
	...
	return packs.Pack{
		...
		Destructive: []packs.Rule{
			{ID: "osascript-send-message", Severity: sevCritical, Confidence: confHigh, Reason: "osascript can send iMessages on your behalf", Remediation: "Review AppleScript before allowing message send", Match: packs.And(packs.Name("osascript"), packs.ArgContentRegex(osascriptMessagesRe.String()))},
			{ID: "osascript-send-email", Severity: sevCritical, Confidence: confHigh, Reason: "osascript can send emails via Mail.app", Remediation: "Review AppleScript before allowing email send", Match: packs.And(packs.Name("osascript"), packs.ArgContentRegex(osascriptMailRe.String()))},
			...
			{ID: "osascript-sensitive-app", Severity: sevHigh, Confidence: confHigh, Reason: "osascript targets a sensitive app", Remediation: "Verify this AppleScript interaction is required", Match: packs.And(packs.Name("osascript"), packs.ArgContentRegex(osascriptSensitiveAppsRe.String()))},
			{ID: "shortcuts-run", Severity: sevHigh, Confidence: confHigh, Reason: "Apple Shortcuts can perform arbitrary automation", Remediation: "Review shortcut actions before running", Match: packs.And(packs.Name("shortcuts"), packs.ArgAt(0, "run"))},
			{ID: "automator-run", Severity: sevHigh, Confidence: confMedium, Reason: "Automator workflows can perform arbitrary automation", Remediation: "Review workflow before running", Match: packs.And(packs.Name("automator"), packs.Not(packs.Or(packs.Flags("--help"), packs.Flags("-h"), packs.Flags("--version"))))},
			{ID: "osascript-finder-destructive", Severity: sevHigh, Confidence: confHigh, Reason: "osascript performs destructive Finder operations", Remediation: "Review Finder operation before allowing", Match: packs.And(packs.Name("osascript"), packs.ArgContentRegex(osascriptFinderDestRe.String()))},
			...
			{ID: "osascript-jxa-catchall", Severity: sevMedium, Confidence: confLow, Reason: "osascript JavaScript mode can automate apps in non-AppleScript syntax", Remediation: "Review JXA payload and app targets before execution", Match: jxaCatchAll},
		},
	}
}
