...
func frameworksPack() packs.Pack {
	return packs.Pack{
		...
		Rules: []packs.Rule{
			...
			{ID: "managepy-flush", Severity: sevHigh, Confidence: confHigh, EnvSensitive: true, Reason: "manage.py flush removes all table data", Remediation: "Use targeted delete commands in management shells", Match: packs.Or(
			...
			{ID: "managepy-migrate-syncdb", Severity: sevMedium, Confidence: confMedium, EnvSensitive: true, Reason: "manage.py migrate --run-syncdb can reset unmanaged schema state", Remediation: "Use standard migrate without --run-syncdb", Match: packs.Or(
			...
			{ID: "artisan-migrate-fresh", Severity: sevHigh, Confidence: confHigh, EnvSensitive: true, Reason: "artisan migrate:fresh drops all tables and reruns migrations", Remediation: "Use php artisan migrate for incremental changes", Match: packs.Or(
			...
			{ID: "mix-ecto-reset", Severity: sevHigh, Confidence: confHigh, EnvSensitive: true, Reason: "mix ecto.reset drops and recreates database state", Remediation: "Use ecto.migrate for incremental changes", Match: packs.And(packs.Name("mix"), packs.Or(packs.ArgAt(0, "ecto.reset"), packs.ArgAt(0, "ecto.drop")))},
		},
	}
}
